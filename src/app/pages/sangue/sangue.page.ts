...
@Component({
  selector: 'app-sangue',
  templateUrl: './sangue.page.html',
  styleUrls: ['./sangue.page.scss'],
})
export class SanguePage implements OnInit {
    ...
    this.Authorizer.QueryStoreProc('Executar', "spExame", params).then(res => {
      ...
      try {
        if (resultado.success) {
          ...
          this.alertService.presentAlert({ pTitle: 'ATENÇÃO', pSubtitle: 'Exame', pMessage: 'Não  paciente' });
        ...
        this.alertService.presentAlert({ pTitle: this.env.APP_NAME, pSubtitle: 'Patient', pMessage: 'Nenhum usuário!' });
      }
    });
