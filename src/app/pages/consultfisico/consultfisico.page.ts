...
@Component({
  ...
  templateUrl: './consultfisico.page.html',
  styleUrls: ['./consultfisico.page.scss'],
})
export class ConsultfisicoPage implements OnInit {
  ...
  GravarDados(form: NgForm) {
    ...
    this.Authorizer.QueryStoreProc('Executar', 'spHospital', params).then(res => {
      ...
      try {
        ...
        this.alertService.presentAlert({ pTitle: this.env.APP_NAME, pSubtitle: 'Exame', pMessage: 'Nenhum usuário!' });
      }
    });
  }
...
console.log('params',params)
    this.Authorizer.QueryStoreProc('Executar', 'spExame', params).then(res => {
      ...
      try {
        ...
        this.alertService.presentAlert({ pTitle: this.env.APP_NAME, pSubtitle: 'Exame', pMessage: 'Não exame' });
      }
    });
